...
def create_app(test_config=None):
    ...
    def summary():
        ...
        if repo_name:
            repos = cur.execute(
                f'SELECT names.name, count(name) FROM (SELECT name FROM Repository WHERE name LIKE "{repo_name}%") names LEFT JOIN Issue on names.name = Issue.repo_name GROUP BY names.name')
    ...
    def rank_all():
        ...
        print(query_params)
        cur.execute(
            f'SELECT repo_name, owner, count(*) as issues, rank() over (ORDER BY count(*) asc) as ranking FROM Issue GROUP BY repo_name, owner LIMIT 20 OFFSET {int(page) * 20}')
        return jsonify({
    ...
    def update():
        ...
        cur.execute(
            f'UPDATE Repository SET stars = {metadata["stars"]} WHERE name = "{repo_name}" AND owner = "{owner}"')
        con.commit()
