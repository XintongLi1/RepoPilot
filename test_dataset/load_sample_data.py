...
files = ['semgrep_json/coconut.json', 'semgrep_json/flask.json']
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        for file_path in data['paths']['scanned']:
            ...
            cur.execute(
                f'INSERT INTO File (name, language, path, repo_name, owner) VALUES ("{name}", "python", "{path}", "{data["repo_name"]}", "{data["owner"]}")')
        ...
        for result in data['results']:
            ...
            cur.execute(
                f'INSERT INTO Issue (check_id, start_line, end_line, category, impact, repo_name, owner, path) VALUES ("{check_id}", {start_line}, {end_line}, "{category}", "{impact}", "{data["repo_name"]}", "{data["owner"]}", "{path}")')
    con.commit()
